{
	...
	run:async function(){
		...
		let runMode = await zn.getSettingValue('RunMode', "BlenderComfyUIMax");
		...
		const options = {
			cwd: `${this.pluginPath}\\Blender_ComfyUI`
		};
		
		try {
			await zn.execute('cmd.exe', ['/c', `.\\python_embeded\\python.exe -u -s ComfyUI\\main.py${useCpu} --windows-standalone-build --use-pytorch-cross-attention`], options, callback);
		} catch (error) {
	...
	addDir:async function(){
		...
		if(isInstalled){
			zn.addDirToClass(this.pluginName, {icon:"Folder", viewName:"ComfyUI根目录", viewPath:`..\\ComfyUI`, dirPath:`\\Products\\${this.pluginName}\\Blender_ComfyUI\\ComfyUI`});
			zn.addDirToClass(this.pluginName, {icon:"Folder", viewName:"模型目录", viewPath:`..\\ComfyUI\\models`, dirPath:`\\Products\\${this.pluginName}\\Blender_ComfyUI\\ComfyUI\\models`});
		}
	},
	
	//设置菜单
	addSetting:async function(){
		const settings = {className:"BlenderComfyUIMax", title:"铁锅炖ComfyUI满血版设置", configList:[
		  {
	        key: 'RunMode',
			label:'启动模式',
			type: 'select',
