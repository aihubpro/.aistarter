# AIStarter 文档索引

> 仓库首页入口，支持中文 / English 快速跳转。

- 快速入口： [中文](zh/1_about.md) | [English](en/1_about.md)



## 目录（中文）
- [简介](zh/1_about.md)
- [下载与安装](zh/2_download-installation.md)
  - [Windows](zh/download-installation/windows.md)
  - [macOS](zh/download-installation/mac.md)
  - [Linux](zh/download-installation/linux.md)
- [使用指南](zh/3_usage-guide.md)
- [应用市场目录](zh/4_marketplace-catalog.md)
- [AI 服务面板](zh/5_ai-server-panel.md)

## Table of Contents (English)
- [About](en/1_about.md)
- [Download & Installation](en/2_download-installation.md)
  - [Windows](en/download-installation/windows.md)
  - [macOS](en/download-installation/mac.md)
  - [Linux](en/download-installation/linux.md)
- [Usage Guide](en/3_usage-guide.md)
- [Marketplace Catalog](en/4_marketplace-catalog.md)
- [AI Server Panel](en/5_ai-server-panel.md)

## 使用说明
- 若本仓库根目录即为文档根（当前结构），上述链接可直接使用。
- 若你将文档放在项目的 `docs/` 子目录下，请在项目根的 `README.md` 使用如下链接：
  - 中文（仓库内文件视图）：`docs/zh/1_about.md`
  - English (Repo file view): `docs/en/1_about.md`
  - 若在项目根 `README.md` 引用，请在链接前加 `docs/` 前缀（示例见上）。