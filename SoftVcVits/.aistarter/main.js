{
	...
	run:async function(){	
		...
		const options = {
			cwd: `${this.pluginPath}\\so-vits-svc-4.1`
		};
	...
	addDir:async function(){
		...
		if(isInstalled){
			zn.addDirToClass(this.pluginName, {icon:"Folder", viewName:"SoftVcVits目录", viewPath:`.\\${this.pluginName}\\so-vits-svc-4.1`, dirPath:`\\Products\\${this.pluginName}\\so-vits-svc-4.1`});
			
			zn.addDirToClass(this.pluginName, {icon:"Folder", viewName:"模型目录", viewPath:`.\\${this.pluginName}\\so-vits-svc-4.1\\models`, dirPath:`\\Products\\${this.pluginName}\\so-vits-svc-4.1\\models`});
			
			zn.addDirToClass(this.pluginName, {icon:"Folder", viewName:"预训练模型", viewPath:`..\\logs\\44k`, dirPath:`\\Products\\${this.pluginName}\\so-vits-svc-4.1\\logs\\44k`});
			
			zn.addDirToClass(this.pluginName, {icon:"Folder", viewName:"扩散模型", viewPath:`..\\logs\\44k\\diffusion`, dirPath:`\\Products\\${this.pluginName}\\so-vits-svc-4.1\\logs\\44k\\diffusion`});
