{
	...
	_initEnv:async function(){
		...
		let rootPath = await zn.getSettingValue('rootDir', 'StableDiffusionAki');
	...
	download:async function(){
		...
		await zn.setSettingValue('rootDir', `${this.pluginPath}\\sd-webui-aki-v4.6.1`, 'StableDiffusionAki');
	...
	run:async function(){
		...
		let fastGup = await zn.getSettingValue('fastGup', 'StableDiffusionAki');
		let fastGupCmd = '';
		if(fastGup){
			fastGupCmd = ' && set SAFETENSORS_FAST_GPU=1';
		}
		
		let openApi = await zn.getSettingValue('openApi', 'StableDiffusionAki');
		let openApiCmd = '';
		if(openApi){
			openApiCmd = ' --api';
		}
	...
	onSettingChange:async function(settingKey, settingValue, settingClass){
		...
		if(settingClass == "StableDiffusionAki" && settingValue){
	...
	addSetting:async function(){
		const settings = {className:"StableDiffusionAki", title:"StableDiffusion绘世版设置", configList:[
		  {
			key: 'rootDir',
			label: '绘世目录',
