...
<% } %>
	getOpenCommand:async function(url) {
...
<% if (run) { %>
	...
	run:async function(){
        ...
        zn.showLoading(true, "正在启动，请耐心等候！",180000); //3分钟后自动关闭loading，结束判断失效时启用
        ...
        let callback = {
        	...
        	stdout:async (data) => { //标准输出  可改 stderr 错误输出
        		if (data.toString().includes("Starting")) { //结束判断
                    zn.showLoading(false); //关闭loading
                    const [command, args] = await this.getOpenCommand('127.0.0.1:8999');
					// zn.execute(command, args) // 项目没有自动跳转，可以手动跳转
        		}
        	},
            stderr:async (data) => { //错误输出  可改 stdout 标准输出
        		if (data.toString().includes("Starting")) { //结束判断
                    zn.showLoading(false); //关闭loading
                    const [command, args] = await this.getOpenCommand('127.0.0.1:8999');
					// zn.execute(command, args) // 项目没有自动跳转，可以手动跳转
        ...
        const options = {
        	cwd: `${this.pluginPath}/`, //cwd是命令执行的目录，例如cmd执行命令的时候，要先用cd指定到目录执行
            env: {
        ...
        <% } %>
        //设置环境变量例子:
        //set HOME=../aaa/bbb&./run.exe
        //set HOME=../aaa/bbb&./run.bat
        //set HOME=../aaa/bbb&./venv/python.exe -u app.py
        //let cmdStr = 'set HOME=../aaa/bbb&./run.exe'
        let cmdStr = './run.bat' //启动命令
        /** 
         * 需要使用变动的命令 如下例子：
         ...
         *     useCpu = 'gpu'
         * }
         * let cmdStr = `python -u app.py ${useCpu}` //这样就可以设置选择启动时运行的是cpu 还是 gpu。 python -u app.py cpu 或者 python -u app.py gpu 执行命令的时候
         * 或者
         * await zn.execute('python', ['-u', `app.py ${useCpu}`], options, callback) //这样就可以设置选择启动时运行的是cpu 还是 gpu。 python -u app.py cpu 或者 python -u app.py gpu 执行命令的时候
         */

        try {
        	await zn.execute('/usr/bin/env', [cmdStr], options, callback) //执行命令 提示：如果需要设置环境变量，请使用set HOME=../aaa/bbb&./run.exe 如果需要变动的值例如 ${useCpu} 这种则需要配合设置菜单使用例如 let useCpu = '' 当设置菜单的值是 cpu 时 useCpu = 'cpu' 当设置菜单的值是 gpu 时 useCpu = 'gpu' 这样就可以设置选择启动时运行的是cpu 还是 gpu
        } catch (error) {
...
<% if (runScript) { %>
	//运行其他脚本，例如：查看配置信息，更新项目等……
    runScript:async function(value){
	    const options = {
			cwd: this.pluginPath
		};
		try{
		    await zn.execute('/usr/bin/env', [value], options)
		}catch(e){
			console.log(e)
		}
	},
...
<% if (addSetting) { %>
	...
	addSetting:async function(){
        ...
        { //按钮
            ...
            btn_icon:"Plus", //按钮图标
            btn_text:'启动', //按钮文本
            btn_style:"success", //(可选) 按钮状态，支持success、danger、warning、info、primary 默认success
...
<% if (onRunBtnClicked) { %>
	...
	onRunBtnClicked:async function(){
		...
		try {
            const [command, args] = await this.getOpenCommand('http://www.baidu.com');
			await zn.execute('/usr/bin/env', [command, args], Options, callback) //启动进程
		} catch (error) {
			console.error(error.message);
			if(this.runbutton){ //判断是否已经启动
