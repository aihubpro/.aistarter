...
<% if (run) { %>
	...
	run:async function(){
        ...
        try {
        	await zn.execute('cmd.exe', ['/c', cmdStr], options, callback) //执行命令 提示：如果需要设置环境变量，请使用set HOME=..\\aaa\\bbb&.\\run.exe 如果需要变动的值例如 ${useCpu} 这种则需要配合设置菜单使用例如 let useCpu = '' 当设置菜单的值是 cpu 时 useCpu = 'cpu' 当设置菜单的值是 gpu 时 useCpu = 'gpu' 这样就可以设置选择启动时运行的是cpu 还是 gpu
        } catch (error) {
...
<% if (runScript) { %>
    ...
    runScript:async function(value){
		...
		try{
		    await zn.execute('cmd.exe', ['/c', 'start '+value], options)
		}catch(e){
			console.log(e)
		}
	},
