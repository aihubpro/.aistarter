              ...
              <div class="user-name ml-2 mr-2 flex">
                ...
                <div class="flex flex-1 justify-end items-center">
                  <div class="like_count mr-2">
                    ...
                    {{ Formatter(item.like_count||0) }}
                  </div>
                  <div class="Collection mr-2">
                    <el-icon>
                      <Star />
                    </el-icon>
                    {{ Formatter(item.favorite_count||0) }}
                  </div>
                  <div class="download">
                    <Download style="width: 1em; height: 1em; margin-right: 1px" />
                    {{ Formatter(item.download||0) }}
                  </div>
                </div>
              </div>
            </div>
...
const { userInfos, openQRCodePayment, openProductDetails } = inject('main') as any;
...
const fetchMoreItems = async () => {
  ...
  let url = (window as any).Constants.uploadUrl + "/users/market-list";
  if(resType.value != 0){ //模型，插件，工作流
    url = (window as any).Constants.uploadUrl + "/users/market-resources-list";
  }
  try {
    const response = await axios.get<any>(url, {
      params: { loadedCount, searchKeyword: searchKeyword.value, tk: localStorage.getItem('token'), orderType: sortValue.value, filters: filter.value,resType: resType.value }
    });
