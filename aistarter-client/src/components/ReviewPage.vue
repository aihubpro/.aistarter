<template>
    <div class="page_bady">
      ...
      <!-- <div class="search_bar">
        <h4 class="title">AI应用市场<span>
            <el-switch v-model="adminReview" class="ml-2" active-text="管理员审核" inactive-text="正常" />
          </span></h4>
        <div class="search_input_wrapper">
          <el-select v-model="value" placeholder="Select" size="small" class="w-[100px]">
      ...
      <!-- 核心内容 -->
      <el-scrollbar class="relative" :height="heightxb ? 'calc(100vh - 100px)' : 'calc(100vh - 140px)'">
        <div class="box-wrapper" v-infinite-scroll="loadMore" infinite-scroll-distance="1" v-if="!hiddenNew">
          ...
          <div class="box-item" v-for="(item, index) in items" :key="index">
            ...
            <div class="relative" @click="openProductDetailsReview(item)">
              <!-- 蒙版已安装 -->
              <div v-for="(item2, index2) in softwareList" :key="index2">
                <div
                  ...
                  v-if="item2.name == item.plugin_name" @click.stop="openProductDetailsReview(item, item2)">
              ...
              <!-- 付费 -->
              <div class="absolute top-0 left-0 bg-black/50 pt-2 pb-2 text-color-yellow font-bold flex"
                style="border-radius: 10px;">
                <div v-if="item.price_type == 2 || (item.price_value > 0 && item.price_type != 1)">￥{{ item.price_value }}</div>
                <!-- 平台 -->
                <div class='ml-2 mr-2' :class='item.platforms == "win" ? "text-color-blue" : "text-color-pink"'>{{ item.platforms }}</div>
              </div>
              <!-- 配置型号和VIP -->
              ...
              <div class="absolute top-0 right-0 bg-black/50 pt-2 pb-2" v-else-if="item.ext_option" style="border-radius: 10px;">
                <div class="flex justify-center items-center">
                  <div class="flex flex-1 justify-center mr-2 ml-2">
                    <div class="w-80px ellipsis">{{ item.ext_option.tag }}</div>
              ...
              <div class="absolute bottom-0 right-0 w-full font-size-3 bg-black/50 pt-1 pb-1">
                ...
                <div class="user-name ml-2 mr-2 flex">
                  ...
                  <div class="flex flex-1 justify-end items-center">
                    <div class="like_count mr-2">
                      <el-icon>
                        <Pointer />
                      </el-icon>
                      {{ Formatter(item.like_count) }}
                    </div>
                    <div class="Collection mr-2">
                      <el-icon>
                        <Star />
                      </el-icon>
                      0
        ...
        <el-result v-if="hiddenNew" class="result-center" style="transform: translate(-50%,100%);" icon="warning"
  ...
  const { userInfos, openQRCodePayment, openProductDetailsReview } = inject('main') as any;
  ...
  const projectType = ref(deepClone(marketFilterData.app))
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      newItems.forEach((item: any) => {
        ...
        //图片处理
        item.image_path = item.image_path ? (window as any).Constants.uploadUrl + "/assets/market-images/" + item.image_path + "?tmpDir=1" : imgurl;
        //配置处理
  ...
  const fetchMoreItems = async () => {
    ...
    let url = (window as any).Constants.uploadUrl + "/users/market-list-review";
    if(resType.value != 0){ //模型，插件，工作流
      url = (window as any).Constants.uploadUrl + "/users/market-resource-list-review";
    }
    try {
      const response = await axios.get<any>(url, {
        params: { loadedCount, searchKeyword: searchKeyword.value, orderType: sortValue.value, filters: filter.value,resType:resType.value },
        headers: {
  ...
  // // 下载
  // const downloadScript = async (item: SoftwareItem) => {
  ...
  //     //会员下载项目
  //     if (store.userInfo.vip_type == 0) {
  //       ElMessage.error("会员专属项目，请先订阅再下载！");
  //       return;
  //     }
  //   }
  ...
  //     //检测是否已经购买
  //     let isBuy = await checkIsBuy(2, fileId);
  ...
  //   matomo.trackEvent("下载脚本", "下载开始", fileId + "|" + intallDir);
  //   // 发送获取插件信息的请求，并处理回复
  //   ipcRenderer.invoke('download-script', fileId, { fileName: intallDir }).then((filePath: string) => {
  //     console.log(filePath);
  //     if (filePath) {
  //       matomo.trackEvent("下载脚本", "下载成功", fileId + "|" + intallDir);
  //     }
  //   }).catch((error: Error) => {
  //     console.error('Error fetching plugin information:', error);
  //   });
  ...
  // // 判断是否购买
  // const checkIsBuy = async (productType: number, productId: number) => {
  //   let url = (window as any).Constants.uploadUrl + "/order/check-is-buy";
  //   try {
  //     const response = await axios.get(url, {
  //       params: { type: productType, id: productId },
  //       headers: {
  //         "access-token": localStorage.getItem('token')
  //       }
  //     });
