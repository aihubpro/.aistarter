            ...
            <el-row :gutter="20" class="w-full overflow-x-hidden">
                <el-col :span="12">
                    <el-form :model="form" label-width="auto" :rules="appRuleFormRules">
                        <el-form-item :label="$t('pluginproject.project_intg_pkg')" prop="importproject">
                            <div @click="handleClick" class="w-full">
                                <el-input
                                    v-model="form.importproject"
                                    :placeholder="$t('pluginproject.project_intg_pkg_pholder')"
                                    :readonly="true"
                                >
                                    <template #append>
                                        <el-icon>
                        ...
                        <el-form-item :label="$t('pluginproject.project_folder')" prop="projectName">
                            <el-tooltip :content="$t('pluginproject.project_folder_tip')" placement="top">
                                <el-input v-model="form.projectName"
                                    :placeholder="$t('pluginproject.project_folder_pholder')" />
                            </el-tooltip>
                        </el-form-item>
                        <el-form-item :label="$t('pluginproject.project_version')" prop="projectVersion">
                            <el-input v-model="form.projectVersion"
                                :placeholder="$t('pluginproject.project_version_pholder')" show-word-limit type="text" />
                        </el-form-item>
                        <el-form-item :label="$t('pluginproject.project_icon')">
                            <el-tooltip
                                effect="dark"
                                :content="$t('pluginproject.project_appicon_text')"
                                placement="top"
                            >
                        ...
                        <el-form-item :label="$t('pluginproject.project_type')" :rules="{ required: true }">
                    ...
                    <el-row :gutter="20" justify="center" class="w-full overflow-x-hidden">
                        <el-col :span="24">
                            <el-form :model="form" :rules="appRuleFormRules">
                                <el-form-item :label="$t('pluginproject.project_title')" prop="projectTitle">
                                    <el-input v-model="form.projectTitle" maxlength="60"
                                        :placeholder="$t('pluginproject.project_title_pholder')" show-word-limit type="text" />
                                </el-form-item>
                            </el-form>
                        </el-col>
                        <el-col :span="24">
                            <el-form :model="form" :rules="appRuleFormRules">
                                <el-form-item :label="$t('pluginproject.project_desc')" prop="projectDesc">
                                    <el-input v-model="form.projectDesc" maxlength="200"
                                        :placeholder="$t('pluginproject.project_desc_pholder')" :autosize="{ minRows: 7, maxRows: 7 }" show-word-limit autosize
                                        type="textarea" />
                                </el-form-item>
                            </el-form>
                        </el-col>
...
//筛选传入类型
const filterbtn = ref(toolutils.deepClone(marketFilterData.plugin).filter((item) => { return !item.hidden }))
...
//     if(newValue=="plugin"&&newValue!=oldValue){
//         resetData()
//     }
// })
...
async function shareValidateDirlocal(rule, value, callback) {
    const isDirExits = await ipcRenderer.invoke('resources-check-plugin-dir-exits-plugin',form.projectName)
    if(isDirExits){
        let emconfirm = await ElMessageBox.confirm(
            t('pluginproject.project_existence_title'),
        ...
        callback(new Error(t('pluginproject.project_existence_title')));
    }else{
        callback();
    }
}
...
const rules = {
    ...
    projectTitle: [
        { ...commonRules, message: t('pluginproject.project_title_message') },
    ],
    projectDesc: [
        { ...commonRules, message: t('pluginproject.project_desc_message') },
    ],
};
...
const resetData = async () => {
    ...
    appIconPath.value = ''
    appIcon.value = ''
    filterbtn.value = await toolutils.deepClone(marketFilterData.plugin).filter((item) => { return !item.hidden })
}
...
const submits = async () => {
    ...
    if(form.importproject){
        let projectipo = await ipcRenderer.invoke('resources-import-integration-plugin', form.projectName,form.importproject)
        if (projectipo) {
    ...
    const reponse = await ipcRenderer.invoke('resources-create-new-plugin-plugin', form.projectName,JSON.stringify(mainJson, null, 2))
    if(reponse){
        console.log(reponse)
        ElMessage({
