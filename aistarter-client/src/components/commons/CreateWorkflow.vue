            ...
            <el-row :gutter="20" class="w-full overflow-x-hidden">
                <el-col :span="12">
                    <el-form :model="form" label-width="auto" :rules="appRuleFormRules">
                        <el-form-item :label="$t('workflowproject.project_intg_pkg')" prop="importproject">
                            <div @click="handleClick" class="w-full">
                                <el-input
                                    v-model="form.importproject"
                                    :placeholder="$t('workflowproject.project_intg_pkg_pholder')"
                                    :readonly="true"
                                >
                                    <template #append>
                                        <el-icon>
                        ...
                        <el-form-item :label="$t('workflowproject.project_folder')" prop="projectName">
                            <el-tooltip :content="$t('workflowproject.project_folder_tip')" placement="top">
                                <el-input v-model="form.projectName"
                                    :placeholder="$t('workflowproject.project_folder_pholder')" />
                            </el-tooltip>
                        </el-form-item>
                        <el-form-item :label="$t('workflowproject.project_version')" prop="projectVersion">
                            <el-input v-model="form.projectVersion"
                                :placeholder="$t('workflowproject.project_version_pholder')" show-word-limit type="text" />
                        </el-form-item>
                        <el-form-item :label="$t('workflowproject.project_icon')">
                            <el-tooltip
                                effect="dark"
                                :content="$t('workflowproject.project_appicon_text')"
                                placement="top"
                            >
                        ...
                        <el-form-item :label="$t('workflowproject.project_type')" :rules="{ required: true }">
                    ...
                    <el-row :gutter="20" justify="center" class="w-full overflow-x-hidden">
                        <el-col :span="24">
                            <el-form :model="form" :rules="appRuleFormRules">
                                <el-form-item :label="$t('workflowproject.project_title')" prop="projectTitle">
                                    <el-input v-model="form.projectTitle" maxlength="60"
                                        :placeholder="$t('workflowproject.project_title_pholder')" show-word-limit type="text" />
                                </el-form-item>
                            </el-form>
                        </el-col>
                        <el-col :span="24">
                            <el-form :model="form" :rules="appRuleFormRules">
                                <el-form-item :label="$t('workflowproject.project_desc')" prop="projectDesc">
                                    <el-input v-model="form.projectDesc" maxlength="200"
                                        :placeholder="$t('workflowproject.project_desc_pholder')" :autosize="{ minRows: 7, maxRows: 7 }" show-word-limit autosize
                                        type="textarea" />
                                </el-form-item>
                            </el-form>
                        </el-col>
...
//筛选传入类型
const filterbtn = ref(toolutils.deepClone(marketFilterData.workflow).filter((item) => { return !item.hidden }))
...
//     if(newValue=="workflow"&&newValue!=oldValue){
//         resetData()
//     }
// })
...
async function shareValidateDirlocal(rule, value, callback) {
    const isDirExits = await ipcRenderer.invoke('resources-check-plugin-dir-exits-workflow', form.projectName)
    if(isDirExits){
        let emconfirm = await ElMessageBox.confirm(
            t('workflowproject.project_existence_title'),
        ...
        callback(new Error(t('workflowproject.project_existence_title')));
    }else{
        callback();
    }
}
...
const rules = {
    ...
    projectTitle: [
        { ...commonRules, message: t('workflowproject.project_title_message') },
    ],
    projectDesc: [
        { ...commonRules, message: t('workflowproject.project_desc_message') },
    ],
};
...
const resetData = async () => {
    ...
    appIconPath.value = ''
    appIcon.value = ''
    filterbtn.value = await toolutils.deepClone(marketFilterData.workflow).filter((item) => { return !item.hidden })
}
...
const submits = async () => {
    ...
    if(form.importproject){
        let projectipo = await ipcRenderer.invoke('resources-import-integration-workflow', form.projectName,form.importproject)
        if (projectipo) {
    ...
    const reponse = await ipcRenderer.invoke('resources-create-new-plugin-workflow', form.projectName,JSON.stringify(mainJson, null, 2))
    if(reponse){
        console.log(reponse)
        ElMessage({
