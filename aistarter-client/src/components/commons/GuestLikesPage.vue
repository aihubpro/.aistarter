...
const fetchMoreItems = async () => {
  ...
  try {
    const response = await axios.get<any>(url, {
      params: { loadedCount, searchKeyword: searchKeyword.value, tk: localStorage.getItem('token'), orderType: sortValue.value, filters: filter.value,resType: resType.value,userId:props.items,likes:true}
    });
