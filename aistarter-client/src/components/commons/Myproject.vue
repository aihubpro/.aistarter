    ...
    <el-card class="w-full">
        <template #header>
            <div class="flex justify-start">
                ...
                <div class="flex justify-center ml-2" style="align-items: end;">
                    <span>{{ $t('myproject.myproject_title') }}</span>
                </div>
            </div>
        </template>
        ...
        <div class="w-full">
            <el-form :inline="true" :model="tableData" class="w-full flex flex-wrap">
                <el-form-item :label="$t('myproject.form_project_query')">
                    <el-input v-model="projectData" :placeholder="$t('myproject.myproject_search')" clearable class="w-30"/>
                </el-form-item>
                <el-form-item :label="$t('myproject.form_platform_query')">
                    <el-select
                      v-model="platformsData"
                      :placeholder="$t('myproject.form_platform_query_placeholder')"
                      clearable
                      class="w-20"
                    >
                ...
                <el-form-item :label="$t('myproject.form_review_status')">
                    <el-select
                      v-model="stateData"
                      :placeholder="$t('myproject.form_review_status_placeholder')"
                      clearable
                      class="w-20"
                    >
                ...
                <el-form-item>
                    <el-button type="primary" @click="onSubmit" size="small">{{ $t('myproject.form_query_button') }}</el-button>
                    <el-button type="primary" @click="onReset" size="small" >{{ $t('myproject.form_reset_button') }}</el-button>
                </el-form-item>
            </el-form>
        </div>
        ...
        <template #footer>
            ...
            <el-table ref="tableRef" v-loading="loading" :data="tableData" :header-row-style="{textAlign: 'center'}" stripe max-height="400px" :scrollbar-always-on="true" :flexible="true" :fit="true">
                ...
                <el-table-column prop="plugin_name" :label="$t('myproject.table_name')" width="140" show-overflow-tooltip align="center"/>
                ...
                <el-table-column fixed="right" :label="$t('myproject.table_operate')" width="110">
                    <template #default="scope">
                        <el-link type="primary" @click="checkDetails({'id':scope.row.id},null,null,scope.row.user_id,scope.row.istmp)">{{$t('myproject.table_operate_look')}}</el-link>
                        <el-link type="primary" class="ml-1" @click="projectDelete(scope.row.id,scope.row.istmp)">{{$t('myproject.table_operate_del')}}</el-link>
                        <!--
...
async function getList() {
    loading.value=true
    let url = (window as any).Constants.uploadUrl + "/users/get-user-project-data";
    try{
...
//项目删除
const projectDelete = (id:any,temp:boolean) =>{
    ElMessageBox.confirm(t('myproject.table_operate_del_tis'), t('myproject.table_operate_del_title'), {
      ...
      type: 'warning',
        ...
        loading.value=true
        let url = (window as any).Constants.uploadUrl + "/users/delete-user-project-data";
        try{
