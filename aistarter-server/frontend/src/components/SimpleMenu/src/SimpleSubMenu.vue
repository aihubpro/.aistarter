  ...
  <MenuItem :name="item.path" v-if="!menuHasChildren(item) && getShowMenu" v-bind="$props" :class="getLevelClass">
    ...
    <template #title>
      <span :class="['ml-2', `${prefixCls}-sub-title`]">
        <span class="relative">
          {{ getI18nName }}
          <span class="menu-red-dot" v-if="getShowNewTag"></span>
        </span>
      </span>
...
<script lang="ts">
  ...
  import { useNewMessageStore } from '/@/store/modules/newMessage'
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const getRealName = computed(() => props.item?.name2) // 获取接口返回的真正name
      const getShowSubTitle = computed(() => !props.collapse || !props.parent)
      const getIsCollapseParent = computed(() => !!props.collapse && !!props.parent)
      
      // 判断是否显示new标签
      const getShowNewTag = computed(() => {
        return getRealName.value && newMessageStore.isNewMessage(getRealName.value)
      })
