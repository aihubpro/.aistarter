<template>
  ...
  <paypal-config-edit ref="paypal" @ok="ok" />
  <stripe-config-edit ref="stripe" @ok="ok" />
...
<script setup lang="ts">
  ...
  import PaypalConfigEdit from '/@/views/payment/channel/paypal/config/PaypalConfigEdit.vue'
  import StripeConfigEdit from '/@/views/payment/channel/stripe/config/StripeConfigEdit.vue'
  ...
  function show(record: PayChannelConfig) {
    switch (record.code) {
      ...
      case payChannelEnum.PAYPAL: {
        paypal.init(record)
        break 
      }
      case payChannelEnum.STRIPE: {
        stripe.init(record)
        break
      }
