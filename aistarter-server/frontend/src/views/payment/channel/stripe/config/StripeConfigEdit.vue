<template>
    <basic-drawer showFooter v-bind="$attrs" width="60%" title="Stripe支付配置" :visible="visible" :maskClosable="false" @close="handleCancel">
      <a-spin :spinning="confirmLoading">
        ...
        >
          ...
          <a-form-item label="可发布密钥" name="publishableKey">
            <a-textarea :rows="2" :disabled="showable" v-model:value="form.publishableKey" placeholder="请输入可发布密钥(pk_开头)" />
          </a-form-item>
          <a-form-item label="私密密钥" name="secretKey">
            <a-textarea :rows="2" :disabled="showable" v-model:value="form.secretKey" placeholder="请输入私密密钥(sk_开头)" />
          </a-form-item>
          <a-form-item label="Webhook密钥" name="webhookSecret">
            <a-textarea :rows="2" :disabled="showable" v-model:value="form.webhookSecret" placeholder="请输入Webhook密钥(可选)" />
          </a-form-item>
          <a-form-item label="货币类型" name="currency">
            <a-select v-model:value="form.currency" :disabled="showable" placeholder="请选择货币类型">
          ...
          <a-form-item name="notifyUrl">
            <template #label>
              <basic-title helpMessage="接收Stripe支付相关的回调数据的地址"> 异步通知地址 </basic-title>
            </template>
  ...
  <script lang="ts" setup>
    ...
    import { getConfig, update, generateNotifyUrl, StripeConfig } from './StripeConfig.api'
    ...
    const rules = computed(() => {
      return {
        publishableKey: [{ required: true, message: '请输入可发布密钥' }],
        secretKey: [{ required: true, message: '请输入私密密钥' }],
