    ...
    <a-spin :spinning="confirmLoading">
      ...
      >
        ...
        <a-form-item name="notifyUrl">
          <template #label>
            <basic-title helpMessage="接收微信支付相关的回调数据的地址"> 异步通知地址 </basic-title>
          </template>
        ...
        <a-form-item label="APIv3证书" name="apiCertV3">
          <a-textarea :rows="3" :disabled="showable" v-model:value="form.apiCertV3" placeholder="请输入APIv3证书" />
        </a-form-item>
        <a-form-item label="APIv3证书密钥" name="apiKeyV3">
          <a-textarea :rows="3" :disabled="showable" v-model:value="form.apiKeyV3" placeholder="请输入APIv3证书密钥" />
        </a-form-item>
      </a-form>
    </a-spin>
    <template #footer>
...
<script lang="ts" setup>
  ...
  import { getConfig, update, generateNotifyUrl, WechatPayConfig } from './WechatPayConfig.api'
  ...
  const rules = computed(() => {
    return {
      ...
      apiCertV3: [{ required: true, message: '输入V3证书' }],
      apiKeyV3: [{ required: true, message: '请输入V3证书秘钥' }],
