...
import { updateAIProject, updateAIProjectFilter, uploadImage } from './MultipleRevisionsList.api'
...
const [register, { closeModal, setModalProps }] = useModalInner((data) => {
    ...
    console.log(JSON.parse(data.public_option).filterData)
    importData.value = JSON.parse(JSON.parse(data.public_option).filterData) as any
    filterForm.value = JSON.parse(JSON.parse(data.public_option).filterData)
...
async function handleOk() {
    ...
    try {
        ...
        prourl.filterData = filterForm.value
        ...
        if (fileList.value) {
            await uploadImage(formData).then(async ({ data }) => {
                console.log("imageuploaddata", data)
                serverImageUrl = data
            })
        }
