<template>
  <BasicModal v-bind="$attrs" @register="register" title="项目详细信息" @cancel="handleCancel" width="800px" :showOkBtn="false" cancelText="关闭">
...
const getFilterTags = (publicOption: string) => {
  ...
  try {
    const parsed = JSON.parse(publicOption);
    const filterData = JSON.parse(parsed.filterData || '{}');
    const tags = [];
    for (const key in filterData) {
      if (Array.isArray(filterData[key])) {
        tags.push(...filterData[key]);
      } else {
        tags.push(filterData[key]);
      }
    }
    return tags.filter(tag => tag && tag !== 'del');
  } catch {
    return [];
  }
};
