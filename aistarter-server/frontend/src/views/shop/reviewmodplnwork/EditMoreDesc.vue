...
import { updateRestmp, uploadImageRestmp } from './Resources.api'
...
const [register, { closeModal, setModalProps }] = useModalInner((data) => {
    ...
    if (data.image_path) {
        dataimg = globSetting.apiUrl+'/assets/market-images/' + data.image_path+ '?tmpDir=1&timetamp='+Date.now(); //表示临时文件目录
    }
    imageUrl.value = dataimg ? dataimg : '/src/assets/images/ai_noimg.png'; //获取新图片
    oldimageUrl.value = data.image_path; //保留旧图片
    public_option.value = data.ext_option; //相关链接
    ...
    form.resInstall = data.res_install;
    setModalProps({ title: `编辑用户 ${data.user_name} 的项目信息` });
    console.log(JSON.parse(JSON.parse(data.ext_option).filterData))
    importData.value = JSON.parse(JSON.parse(data.ext_option).filterData) as any
    filterForm.value = JSON.parse(JSON.parse(data.ext_option).filterData)
    ...
    filterbtn.value.forEach((item) => {
        ...
        if (importData.value && importData.value[item.id]) {
            item.projectModel = importData.value[item.id]
        }
...
async function handleOk() {
    ...
    try {
        ...
        let serverImageUrl
        if (fileList.value) {
            await uploadImageRestmp(formData).then(async ({ data }) => {
                serverImageUrl = data
            })
        }
