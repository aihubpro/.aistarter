  ...
  <div>
    ...
    <div class="m-3 p-3 bg-white">
      ...
      >
        ...
        <vxe-column field="res_name" title="项目名称" :min-width="230" />
        <vxe-column field="short_desc" title="描述" :min-width="220">
          <template #default="{ row }"> {{ row.short_desc }} </template>
        </vxe-column>
        <vxe-column field="version" title="版本" :min-width="130" />
        <vxe-column field="platforms" title="平台" :min-width="80" />
        <vxe-column field="res_type" title="类型" :min-width="80" >
          <template #default="{ row }">
            <Tag v-if="row.res_type == 1" color="blue">模型</Tag>
            <Tag v-if="row.res_type == 2" color="blue">插件</Tag>
            <Tag v-if="row.res_type == 3" color="blue">工作流</Tag>
          </template>
        </vxe-column>
        ...
        <vxe-column field="res_zip_size" title="文件大小" :min-width="150" >
          <template #default="{ row }"> {{ formatFileSize(row.res_zip_size) }} </template>
        </vxe-column>
    ...
    <ResourcesViewModal @register="registerView" />
  </div>
</template>
...
<script lang="ts" setup>
  ...
  import { page,delRestmp,setResstate } from './Resources.api'
  import { useModal } from '/@/components/Modal';
  import EditMoreDesc from './EditMoreDesc.vue'
  import ResourcesViewModal from './ResourcesViewModal.vue'
  ...
  let projectTypeList = $ref<LabeledValue[]>([
    {key:"res_type", value:1, label:"模型"}, 
    {key:"res_type", value:2, label:"插件"},
    {key:"res_type", value:3, label:"工作流"}
  ])
  ...
  const fields = computed(() => {
    return [
      { field: 'res_name', type: STRING, name: '资源名称', placeholder: '请输入资源名称' },
      ...
      { field: 'res_type',type: LIST, name: '类型', placeholder: '请选择类型', selectList: projectTypeList },
  ...
  function delProject(record){
    ...
    createConfirm({
      iconType: 'warning',
      title: '警告',
      content: `是否删除【${record.res_name}】该项目，删除将不可恢复！`,
      onOk: () => {
          delRestmp({id:record.id}).then(() => {
  ...
  function setState(record, state){
    setResstate({id:record.id, state}).then(async (res) => {
